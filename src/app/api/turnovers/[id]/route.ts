...
export async function PATCH(
  ...
  if (body.status !== undefined) {
    if (!["open", "in_progress", "complete"].includes(body.status)) {
      return NextResponse.json(
  ...
  if (body.departing_guest_ref !== undefined) {
    updates.departingGuestRef = body.departing_guest_ref?.trim() || null;
  }

  if (body.arriving_guest_ref !== undefined) {
    updates.arrivingGuestRef = body.arriving_guest_ref?.trim() || null;
  }
